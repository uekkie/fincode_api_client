...
module FincodeApiClient
  class PaymentKonbiniExecutingRequest < ApiModelBase
    ...
    # 支払い期限日数\\ この決済の支払い期限日数です。`0`日から`14`日まで指定できます。  - `0`を指定すると決済を作成した当日に限り支払いが可能な決済が作成されます。 - デフォルト：`2` 
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `FincodeApiClient::PaymentKonbiniExecutingRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `FincodeApiClient::PaymentKonbiniExecutingRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@payment_term_day.nil? && @payment_term_day.to_s.length > 2
        invalid_properties.push('invalid value for "payment_term_day", the character length must be smaller than or equal to 2.')
      end
      ...
      if @pixel_ratio.to_s.length < 1
        invalid_properties.push('invalid value for "pixel_ratio", the character length must be greater than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if !@payment_term_day.nil? && @payment_term_day.to_s.length > 2
    ...
    def payment_term_day=(payment_term_day)
      ...
      if payment_term_day.to_s.length > 2
        fail ArgumentError, 'invalid value for "payment_term_day", the character length must be smaller than or equal to 2.'
      end

      if payment_term_day.to_s.length < 1
    ...
    def pixel_ratio=(pixel_ratio)
      ...
      if pixel_ratio.to_s.length < 1
        fail ArgumentError, 'invalid value for "pixel_ratio", the character length must be greater than or equal to 1.'
      end
    ...
    # Custom attribute writer method with validation
    # @param [Object] win_size_type Value to be assigned
    def win_size_type=(win_size_type)
      if win_size_type.nil?
        fail ArgumentError, 'win_size_type cannot be nil'
      end
    ...
    def hash
      [pay_type, access_id, customer_id, payment_term_day, device_name, win_width, win_height, pixel_ratio, win_size_type].hash
    end
