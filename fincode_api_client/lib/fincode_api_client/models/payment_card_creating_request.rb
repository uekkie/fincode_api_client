...
module FincodeApiClient
  class PaymentCardCreatingRequest
    ...
    # 利用金額\\ `tax`との合計値が決済手段ごとに定められた決済金額の範囲内に収まる必要があります。\\ 参照： [カード決済 > 制限事項 > 利用可能な金額](https://docs.fincode.jp/payment/restriction) 
    attr_accessor :amount

    # 税送料\\ `amount`との合計値が決済手段ごとに定められた決済金額の範囲内に収まる必要があります。\\ 参照： [カード決済 > 制限事項 > 利用可能な金額](https://docs.fincode.jp/payment/restriction) 
    attr_accessor :tax
    ...
    def initialize(attributes = {})
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `FincodeApiClient::PaymentCardCreatingRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
