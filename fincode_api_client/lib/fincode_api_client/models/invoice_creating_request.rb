...
module FincodeApiClient
  class InvoiceCreatingRequest < ApiModelBase
    # インボイスID      ショップ内で一意な値を設定してください。   未指定の場合自動生成します。 
    attr_accessor :id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `FincodeApiClient::InvoiceCreatingRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `FincodeApiClient::InvoiceCreatingRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [id, bill_mail_send_flag, receipt_mail_send_flag, underpayment_mail_send_flag, invoice_number, customer_id, customer_honorific, customer_overwrite, issuer_overwrite, lines, pay_types, card, virtual_account, client_field_1, client_field_2, client_field_3].hash
    end
