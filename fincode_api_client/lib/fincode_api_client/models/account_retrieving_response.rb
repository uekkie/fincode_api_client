...
module FincodeApiClient
  class AccountRetrievingResponse < ApiModelBase
    # 売上入金ID 
    attr_accessor :id
    ...
    # 売上入金 精算期間中に発生した、プラットフォームが設定した振替口座のWeb登録手数料\\ ※ テナントのみ 
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `FincodeApiClient::AccountRetrievingResponse` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `FincodeApiClient::AccountRetrievingResponse`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@id.nil? && @id.to_s.length < 32
        invalid_properties.push('invalid value for "id", the character length must be greater than or equal to 32.')
      end
    ...
    def id=(id)
      ...
      if id.to_s.length < 32
        fail ArgumentError, 'invalid value for "id", the character length must be greater than or equal to 32.'
      end
    ...
    def hash
      [id, account_id, shop_id, deposit_amount, deposit_date, scheduled_deposit_date, aggregate_term_start, aggregate_term_end, payment_deadline, payment_completion_date, status_code, count, settlement_amount, bank_transfer_fee, total_amount, fee_amount, taxable_fee_amount, nontaxable_fee_amount, web_registration_fee_amount, tax_amount, platform_fee_amount, platform_fee_tax_amount, platform_web_registration_fee_amount, created, updated, deposit_destination].hash
    end
