...
module FincodeApiClient
  class RetrievePaymentListQueryParameterOneOf < ApiModelBase
    ...
    def self.openapi_all_of
      ...
      :'PaymentListRetrievingCardQueryParams',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `FincodeApiClient::RetrievePaymentListQueryParameterOneOf` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `FincodeApiClient::RetrievePaymentListQueryParameterOneOf`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [pay_type, keyword, total_amount_min, total_amount_max, customer_id, process_date_from, process_date_to, status, auth_max_date_from, auth_max_date_to, subscription_id, pay_pattern, update_date_from, update_date_to, sort, page, limit, count_only].hash
    end
