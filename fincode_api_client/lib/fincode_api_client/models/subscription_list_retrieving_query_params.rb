...
module FincodeApiClient
  class SubscriptionListRetrievingQueryParams < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `FincodeApiClient::SubscriptionListRetrievingQueryParams` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `FincodeApiClient::SubscriptionListRetrievingQueryParams`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [pay_type, id, customer_id, plan_id, status, total_amount_min, total_amount_max, interval_pattern, start_date_from, start_date_to, stop_date_from, stop_date_to, next_charge_date_from, next_charge_date_to, update_date_from, update_date_to, sort].hash
    end
