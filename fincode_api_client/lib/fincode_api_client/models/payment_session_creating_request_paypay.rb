...
module FincodeApiClient
  ...
  class PaymentSessionCreatingRequestPaypay < ApiModelBase
    # この決済URL上でPayPayによる決済を行った際、仮売上にするか即座に売上確定するか指定します。  - `AUTH`：仮売上にする - `CAPTURE`：即時売上 
    attr_accessor :job_code

    # 注文内容の説明\\ PayPayアプリ上で注文内容として表示されます。 
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `FincodeApiClient::PaymentSessionCreatingRequestPaypay` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `FincodeApiClient::PaymentSessionCreatingRequestPaypay`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [job_code, order_description].hash
    end
