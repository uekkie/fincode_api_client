...
module FincodeApiClient
  class CustomerPaymentMethodCreatingRequest < ApiModelBase
    ...
    def self.openapi_types
      {
        ...
        :'card' => :'CustomerPaymentMethodCreatingRequestCard',
        :'directdebit' => :'CustomerPaymentMethodCreatingRequestDirectdebit'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `FincodeApiClient::CustomerPaymentMethodCreatingRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `FincodeApiClient::CustomerPaymentMethodCreatingRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [pay_type, default_flag, return_url, return_url_on_failure, client_field_1, client_field_2, client_field_3, card, directdebit].hash
    end
