  ...
  class PaymentCard
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `FincodeApiClient::PaymentCard` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `FincodeApiClient::PaymentCard`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [pay_type, job_code, status, card_id, brand, card_no, expire, holder_name, card_no_hash, method, pay_times, bulk_payment_id, subscription_id, bill_id, tds_type, tds2_type, tds2_ret_url, return_url, return_url_on_failure, tds2_status, merchant_name, forward, issuer, transaction_id, approve, auth_max_date, item_code, send_url, acs].hash
    end
