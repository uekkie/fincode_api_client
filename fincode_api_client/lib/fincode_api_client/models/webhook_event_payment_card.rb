...
module FincodeApiClient
  class WebhookEventPaymentCard < ApiModelBase
    # ショップID 
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `FincodeApiClient::WebhookEventPaymentCard` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `FincodeApiClient::WebhookEventPaymentCard`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def bulk_payment_id=(bulk_payment_id)
      if bulk_payment_id.nil?
        fail ArgumentError, 'bulk_payment_id cannot be nil'
      end

      if bulk_payment_id.to_s.length > 25
    ...
    def hash
      [shop_id, order_id, access_id, pay_type, status, customer_id, method, pay_times, client_field_1, client_field_2, client_field_3, amount, tax, currency, job_code, subscription_id, bulk_payment_id, forward, transaction_id, transaction_date, approve, error_code, event].hash
    end
