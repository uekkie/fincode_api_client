...
module FincodeApiClient
  class PlanCreatingRequest < ApiModelBase
    # プランID 
    attr_accessor :id
    ...
    # 利用金額\\ `tax`との合計値が決済手段ごとに定められた決済金額の範囲内に収まる必要があります。 
    attr_accessor :amount

    # 税送料\\ `amount`との合計値が決済手段ごとに定められた決済金額の範囲内に収まる必要があります。 
    attr_accessor :tax
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `FincodeApiClient::PlanCreatingRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `FincodeApiClient::PlanCreatingRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      return false if @plan_name.to_s.length > 200
    ...
    def hash
      [id, plan_name, description, amount, tax, interval_pattern, interval_count].hash
    end
