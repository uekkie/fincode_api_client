...
module FincodeApiClient
  class PaymentGooglePayExecutingResponse < ApiModelBase
    # ショップID 
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `FincodeApiClient::PaymentGooglePayExecutingResponse` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `FincodeApiClient::PaymentGooglePayExecutingResponse`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [shop_id, id, access_id, amount, tax, total_amount, client_field_1, client_field_2, client_field_3, process_date, customer_id, customer_group_id, error_code, created, updated, pay_type, job_code, status, brand, card_no, expire, card_no_hash, method, pay_times, tds_type, tds2_type, tds2_ret_url, tds2_status, merchant_name, forward, issuer, transaction_id, approve, auth_max_date, acs, item_code, return_url, return_url_on_failure, acs_url, redirect_url].hash
    end
