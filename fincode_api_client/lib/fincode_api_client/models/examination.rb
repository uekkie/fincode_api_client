...
module FincodeApiClient
  class Examination < ApiModelBase
    ...
    def self.openapi_types
      {
        :'examination_master_id' => :'ContractExaminationMasterId',
    ...
    def initialize(attributes = {})
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `FincodeApiClient::Examination`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@member_code.nil? && @member_code.to_s.length < 1
        invalid_properties.push('invalid value for "member_code", the character length must be greater than or equal to 1.')
      end
    ...
    def member_code=(member_code)
      ...
      if !member_code.nil? && member_code.to_s.length < 1
        fail ArgumentError, 'invalid value for "member_code", the character length must be greater than or equal to 1.'
      end
