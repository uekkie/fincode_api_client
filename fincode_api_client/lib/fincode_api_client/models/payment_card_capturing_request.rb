...
module FincodeApiClient
  class PaymentCardCapturingRequest < ApiModelBase
    ...
    # 支払方法\\ `CANCELED`ステータスの決済に対して指定ができます。  - `1`: 一括払い - `2`: 分割払い - `5`: リボ払い
    attr_accessor :method

    # （分割払いの場合）支払回数\\ `CANCELED`ステータスの決済に対して指定ができます。\\ \\ 指定可能な値： `3`, `5`, `6`, `10`, `12`, `15`, `18`, `20`, `24` 
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `FincodeApiClient::PaymentCardCapturingRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `FincodeApiClient::PaymentCardCapturingRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [pay_type, access_id, method, pay_times].hash
    end
