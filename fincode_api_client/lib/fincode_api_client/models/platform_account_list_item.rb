...
module FincodeApiClient
  class PlatformAccountListItem < ApiModelBase
    # プラットフォーム利用料収入ID 
    attr_accessor :id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `FincodeApiClient::PlatformAccountListItem` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `FincodeApiClient::PlatformAccountListItem`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [id, account_id, shop_id, deposit_amount, deposit_date, scheduled_deposit_date, aggregate_term_start, aggregate_term_end, payment_deadline, payment_completion_date, status_code, count, settlement_amount, bank_transfer_fee, total_amount, fee_amount, platform_fee_amount, platform_fee_tax_amount, tax_amount, created, updated, verified].hash
    end
