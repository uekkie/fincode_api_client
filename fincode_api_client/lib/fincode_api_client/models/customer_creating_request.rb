...
module FincodeApiClient
  class CustomerCreatingRequest < ApiModelBase
    # 顧客ID\\ 任意の値を指定できます。指定しない場合はfincodeが自動的に生成します。\\ 一度fincodeに登録された顧客IDは削除後も利用できません。 
    attr_accessor :id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `FincodeApiClient::CustomerCreatingRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `FincodeApiClient::CustomerCreatingRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [id, name, email, phone_cc, phone_no, addr_country, addr_state, addr_city, addr_line_1, addr_line_2, addr_line_3, addr_post_code].hash
    end
