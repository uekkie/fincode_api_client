...
module FincodeApiClient
  class RetrievePaymentBulkListQueryParameter < ApiModelBase
    # ページ番号
    attr_accessor :page
    ...
    def self.openapi_all_of
      ...
      :'PaymentBulkListRetrievingQueryParams'
      ]
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `FincodeApiClient::RetrievePaymentBulkListQueryParameter` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `FincodeApiClient::RetrievePaymentBulkListQueryParameter`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [page, limit, count_only, process_plan_date_from, process_plan_date_to, status, pay_type, file_name, delete_flag, created_from, created_to, sort].hash
    end
