...
module FincodeApiClient
  class WebhookEventCustomerPaymentMethod < ApiModelBase
    # ショップID 
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `FincodeApiClient::WebhookEventCustomerPaymentMethod` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `FincodeApiClient::WebhookEventCustomerPaymentMethod`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [shop_id, customer_id, payment_method_id, pay_type, status, client_field_1, client_field_2, client_field_3, application_type, event].hash
    end
