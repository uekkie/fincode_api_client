...
module FincodeApiClient
  # 上書き顧客（請求先）情報 
  class InvoiceCustomerOverwrite < ApiModelBase
    # 上書き顧客（請求先）宛名 
    attr_accessor :name
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `FincodeApiClient::InvoiceCustomerOverwrite` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `FincodeApiClient::InvoiceCustomerOverwrite`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def addr_post_code=(addr_post_code)
      ...
      if addr_post_code.to_s.length > 16
    ...
    def hash
      [name, email, addr_country, addr_state, addr_city, addr_line_1, addr_line_2, addr_line_3, addr_post_code].hash
    end
