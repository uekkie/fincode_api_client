...
module FincodeApiClient
  class PaymentBulkDetail < ApiModelBase
    # 一括決済ID 
    attr_accessor :id
    ...
    # 決済ステータス  - `CHECKED`: チェック済み<br /><span class=\"smallText\">データのチェックが完了し、決済処理の実行を待っています。</span> - `SUCCEEDED`: 決済成功<br /><span class=\"smallText\">この決済の処理においてエラーが発生せず請求が完了しました。</span> - `FAILED`: 決済失敗<br /><span class=\"smallText\">この決済の処理において1件以上のエラーが発生しました。</span> 
    ...
    def initialize(attributes = {})
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `FincodeApiClient::PaymentBulkDetail`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [id, shop_id, order_id, status, access_id, job_code, amount, tax, total_amount, method, pay_times, customer_id, card_id, client_field_1, client_field_2, client_field_3, error_code, created, updated, item_code, transaction_id, approve, forward].hash
    end
