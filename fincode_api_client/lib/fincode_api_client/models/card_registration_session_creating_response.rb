...
module FincodeApiClient
  class CardRegistrationSessionCreatingResponse < ApiModelBase
    # リダイレクト型カード登録URL ID 
    attr_accessor :id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `FincodeApiClient::CardRegistrationSessionCreatingResponse` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `FincodeApiClient::CardRegistrationSessionCreatingResponse`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def link_url=(link_url)
      ...
      if link_url.to_s.length < 1
        fail ArgumentError, 'invalid value for "link_url", the character length must be greater than or equal to 1.'
      end
