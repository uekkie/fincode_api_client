...
module FincodeApiClient
  class PaymentPayPayExecutingRequest < ApiModelBase
    ...
    # ユーザーエージェント\\ リクエストを送信したユーザーエージェントの情報。`redirect_type`が`1`（Webページ／Webアプリケーション）の場合指定できます。 
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `FincodeApiClient::PaymentPayPayExecutingRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `FincodeApiClient::PaymentPayPayExecutingRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @redirect_url.to_s.length > 256
      ...
      if !@user_agent.nil? && @user_agent.to_s.length > 256
        invalid_properties.push('invalid value for "user_agent", the character length must be smaller than or equal to 256.')
      end

      if !@user_agent.nil? && @user_agent.to_s.length < 1
        invalid_properties.push('invalid value for "user_agent", the character length must be greater than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if @redirect_url.to_s.length > 256
      return false if @redirect_url.to_s.length < 1
      return false if !@user_agent.nil? && @user_agent.to_s.length > 256
      return false if !@user_agent.nil? && @user_agent.to_s.length < 1
      true
    end
    ...
    def user_agent=(user_agent)
      ...
      if user_agent.to_s.length > 256
        fail ArgumentError, 'invalid value for "user_agent", the character length must be smaller than or equal to 256.'
      end

      if user_agent.to_s.length < 1
        fail ArgumentError, 'invalid value for "user_agent", the character length must be greater than or equal to 1.'
      end
    ...
    def hash
      [pay_type, access_id, customer_id, redirect_url, redirect_type, user_agent].hash
    end
