...
module FincodeApiClient
  class InvoiceDetailUpdatingRequest < ApiModelBase
    # 回収困難フラグ
    ...
    def initialize(attributes = {})
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `FincodeApiClient::InvoiceDetailUpdatingRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@issue_date.nil? && @issue_date.to_s.length < 10
        invalid_properties.push('invalid value for "issue_date", the character length must be greater than or equal to 10.')
      end
    ...
    def issue_date=(issue_date)
      ...
      if issue_date.to_s.length < 10
        fail ArgumentError, 'invalid value for "issue_date", the character length must be greater than or equal to 10.'
      end
