...
module FincodeApiClient
  class SubscriptionCreatingRequest
    ...
    # （`pay_type = Card`のみ）\\ このサブスクリプションで請求先となるカードのIDです。\\ 省略した場合、顧客のデフォルトカードが使用されます。 
    attr_accessor :card_id

    # （`pay_type = Directdebit`のみ）\\ このサブスクリプションで請求先となる決済手段のIDです。\\ 省略した場合、顧客のデフォルト決済手段が使用されます。 
    attr_accessor :payment_method_id

    # 課金開始日\\ サブスクリプションの開始日です。\\ 登録が請求受付終了日の13:00を超える場合、課金開始日は翌月の同日となります。\\ \\ 形式：`yyyy/MM/dd` 
    ...
    # （`pay_type = Directdebit`のみ）ご利用明細表示内容  - フォーマット：半角英数カナ／全角英数カナ／一部の記号／半角全角スペース - デフォルト： `ショップ名カナの先頭9文字`  ※ 利用可能な記号  - 半角：`.`、`(`、`)`、`–` - 全角：`．`、`（`、`）`、`ー` 
