...
module FincodeApiClient
  ...
  class PaymentSessionCreatingRequestTransaction < ApiModelBase
    # 決済手段\\ この決済URL上で使用できる決済手段を指定します。複数指定可能です。  - `Card`：カード決済 - `Konbini`：コンビニ決済 - `Paypay`：PayPay - `Virtualaccount`：バーチャル口座決済 
    attr_accessor :pay_type

    # 決済金額\\ 決済金額を指定します。\\ \\ 指定可能な金額の範囲は各決済手段ごとの制限事項に準じます。 
    attr_accessor :amount

    # オーダーID\\ このリダイレクト型決済URL上で決済後生成される決済情報のIDを予め指定できます。 
    attr_accessor :order_id

    # 税送料\\ 税送料を指定します。\\ \\ 指定可能な金額の範囲は各決済手段ごとの制限事項に準じます。 
    attr_accessor :tax
    ...
    def self.openapi_types
      {
        :'pay_type' => :'Array<PaymentSessionPayType>',
        :'amount' => :'String',
        :'order_id' => :'String',
        :'tax' => :'String',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `FincodeApiClient::PaymentSessionCreatingRequestTransaction` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `FincodeApiClient::PaymentSessionCreatingRequestTransaction`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'pay_type')
        if (value = attributes[:'pay_type']).is_a?(Array)
          self.pay_type = value
        end
      end
    ...
    def hash
      [pay_type, amount, order_id, tax, client_field_1, client_field_2, client_field_3].hash
    end
