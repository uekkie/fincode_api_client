...
module FincodeApiClient
  # 発行元事業者情報 
  class InvoiceIssuer < ApiModelBase
    # 発行元事業者名 
    attr_accessor :name
    ...
    def initialize(attributes = {})
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `FincodeApiClient::InvoiceIssuer`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@invoice_registration_number.nil? && @invoice_registration_number.to_s.length < 1
        invalid_properties.push('invalid value for "invoice_registration_number", the character length must be greater than or equal to 1.')
      end
    ...
    def invoice_registration_number=(invoice_registration_number)
      ...
      if invoice_registration_number.to_s.length < 1
        fail ArgumentError, 'invalid value for "invoice_registration_number", the character length must be greater than or equal to 1.'
      end
