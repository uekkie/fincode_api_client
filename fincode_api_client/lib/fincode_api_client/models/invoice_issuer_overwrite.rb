...
module FincodeApiClient
  # 上書き発行元事業者情報 
  class InvoiceIssuerOverwrite < ApiModelBase
    # 発行元事業者住所の都道府県コード 
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `FincodeApiClient::InvoiceIssuerOverwrite` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `FincodeApiClient::InvoiceIssuerOverwrite`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [addr_state, addr_city, addr_line_1, addr_line_2, addr_line_3, addr_post_code, email, phone_number].hash
    end
