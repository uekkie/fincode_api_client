...
module FincodeApiClient
  class PaymentVirtualAccountCreatingRequest < ApiModelBase
    # オーダーID\\ 決済情報のIDです。 
    attr_accessor :id
    ...
    # 振込依頼金額\\ `billing_tax`との合計値が決済手段ごとに定められた決済金額の範囲内に収まる必要があります。\\ 参照例： [銀行振込（バーチャル口座） > 制限事項 > 利用可能な金額](https://docs.fincode.jp/payment/virtualaccount/restriction) 
    attr_accessor :billing_amount

    # 振込依頼金額（税送料）\\ `billing_amount`との合計値が決済手段ごとに定められた決済金額の範囲内に収まる必要があります。\\ 参照例： [銀行振込（バーチャル口座） > 制限事項 > 利用可能な金額](https://docs.fincode.jp/payment/virtualaccount/restriction) 
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `FincodeApiClient::PaymentVirtualAccountCreatingRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `FincodeApiClient::PaymentVirtualAccountCreatingRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @billing_amount.to_s.length > 10
        invalid_properties.push('invalid value for "billing_amount", the character length must be smaller than or equal to 10.')
      end

      if @billing_amount.to_s.length < 1
        invalid_properties.push('invalid value for "billing_amount", the character length must be greater than or equal to 1.')
      end

      if !@billing_tax.nil? && @billing_tax.to_s.length > 10
        invalid_properties.push('invalid value for "billing_tax", the character length must be smaller than or equal to 10.')
      end

      if !@billing_tax.nil? && @billing_tax.to_s.length < 1
        invalid_properties.push('invalid value for "billing_tax", the character length must be greater than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if @billing_amount.to_s.length > 10
      return false if @billing_amount.to_s.length < 1
      return false if !@billing_tax.nil? && @billing_tax.to_s.length > 10
      return false if !@billing_tax.nil? && @billing_tax.to_s.length < 1
    ...
    def billing_amount=(billing_amount)
      ...
      if billing_amount.to_s.length > 10
        fail ArgumentError, 'invalid value for "billing_amount", the character length must be smaller than or equal to 10.'
      end

      if billing_amount.to_s.length < 1
        fail ArgumentError, 'invalid value for "billing_amount", the character length must be greater than or equal to 1.'
      end
    ...
    def billing_tax=(billing_tax)
      if !billing_tax.nil? && billing_tax.to_s.length > 10
        fail ArgumentError, 'invalid value for "billing_tax", the character length must be smaller than or equal to 10.'
      end

      if !billing_tax.nil? && billing_tax.to_s.length < 1
        fail ArgumentError, 'invalid value for "billing_tax", the character length must be greater than or equal to 1.'
      end
    ...
    def hash
      [id, pay_type, billing_amount, billing_tax, client_field_1, client_field_2, client_field_3].hash
    end
